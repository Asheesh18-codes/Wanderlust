...
// Fix: Load .env from parent directory (root of project)
...
console.log("DB_URL:", DB_URL); // Debug log
console.log("Token exists:", !!token); // Debug log
console.log("Token value:", token.substring(0, 10) + "..."); // Show first 10 chars

// Fix: More flexible token validation
if (!token || token.length < 20) {
  console.error("Valid Mapbox token required. Set MAPBOX_TOKEN or MAP_TOKEN.");
  process.exit(1);
}

// Remove the pk. check since your token is valid
...
(async () => {
  try {
    ...
    let defaultUser = await User.findOne({ email: "mofane8980@dosonex.com" });
    if (!defaultUser) {
      console.log("Creating default seed user...");
      defaultUser = new User({
        email: "mofane8980@dosonex.com",
        username: "Ashish Singh"
      });
      await User.register(defaultUser, "Ashish123");
    ...
    const docs = [];
    console.log("Processing listings with geocoding...");
    
    for (let i = 0; i < sampleListings.length; i++) {
      const seed = sampleListings[i];
      ...
      try {
        ...
        if (resp.body.features?.length) {
          ...
          console.log(`✅ Geocoded: ${seed.title}`);
        } else {
          console.warn(`⚠️  No geocode result: ${seed.title}`);
        }
      } catch (err) {
        console.warn(`❌ Geocode error for ${seed.title}:`, err.message);
      }
      ...
      docs.push({ 
        ...seed, 
        geometry,
        owner: defaultUser._id  // Add the owner field
      });
      await new Promise(r => setTimeout(r, 150)); // rate limit
    }

    console.log("Inserting listings into database...");
    const insertResult = await Listing.insertMany(docs);
    console.log(`✅ Successfully seeded ${insertResult.length} listings to Atlas!`);
